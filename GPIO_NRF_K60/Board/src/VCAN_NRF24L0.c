...
uint8 TX_ADDRESS[5] = { 0x00,0x00,0x00,0x00,0x01};   // 定义一个静态发送地址
uint8 RX_ADDRESS[5] = { 0x00,0x00,0x00,0x00,0x02};
...
uint8 nrf_init(void)
{
    ...
    spi_init(NRF_SPI, NRF_CS, MASTER,115200);                     //初始化SPI,主机模式
...
void nrf_handler(void)
{
    ...
    if(state & MAX_RT)      //发送超时
    {
        ...
        //printf("time_out!!!! state is %d.TX_DS is %d, RX_DR is %d\n", state, TX_DS, RX_DR);


        //printf("\nMAX_RT state:0x%x", state);
    }
